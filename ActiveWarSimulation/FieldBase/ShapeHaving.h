...
public:
	...
	const std::vector<std::shared_ptr<ShapeHaving>> InShapeHavingList(std::shared_ptr<ShapeHaving> *pShapeHavingVec,size_t vecSize)const;//pShapeVecにある図形のうちthis内に存在する図形を全てリストアップする
	void UpdatePosition(std::shared_ptr<ShapeHaving> *pShapeHavingVec,size_t vecSize,size_t updateTimes);//当たり判定による位置補正をupdateTimes回だけ行う。全ての図形に対しまとめて行うのが前提。std::vectorで管理している場合はstd::vector::data()を用いる。
