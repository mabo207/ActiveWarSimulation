...
public:
	...
	virtual int RubricEvaluate(const std::vector<BattleObject *> &field,const Vector2D stageSize,const std::shared_ptr<const LogElement> &evaluateLog)const=0;
	virtual int InAdvanceDataEvaluate(const std::shared_ptr<InAdvanceCalculateData> &,const std::vector<BattleObject *> &field,const Vector2D stageSize,const std::shared_ptr<const LogElement> &evaluateLog)const{
		//事前計算データを用いて高速評価する関数
