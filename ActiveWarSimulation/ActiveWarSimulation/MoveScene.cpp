...
void MoveScene::thisDraw()const{
	...
	if(keyboard_get(KEY_INPUT_0)<=0){
		...
		//狙っているユニットの描画
		if(m_aimedUnit!=nullptr){
			m_aimedUnit->DrawUnit(Vector2D(),m_battleSceneData->m_fpsMesuring.GetFlame(),true);//アイコンの描画
		}
		...
		//操作中ユニットの描画
		m_battleSceneData->m_operateUnit->DrawUnit(Vector2D(),m_battleSceneData->m_fpsMesuring.GetFlame(),true);
